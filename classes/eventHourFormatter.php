...
class htmlEventHourFormatter implements eventHourFormatter {
    private $numCameras = -1 ;
    
    private function getNumCameras() {
        return $this->numCameras;
    }

    public function setNumCameras($numCameras) {
    ...
    public function format(\eventHour $anHour):string {
        ...
        for ($nCount = 1 ; $nCount <= $this->getNumCameras() ; $nCount++){
            $camera_cells[$nCount] = "" ;
        }
...
class eventHourFormatUtils extends formatUtils {
    ...
    public static function formatEventHour(int $type, eventHour $anHour, int $numCameras = -1) {
        switch ($type) {
        ...
        $formatter = self::createEventHourFormatter($type, $numCameras) ;
        return ($formatter->format($anHour)) ;
    }
    
    private static function createEventHourFormatter(int $type, int $numCameras) {
        switch ($type) {
            case formatUtils::FORMAT_TEXT:
            ...
            case formatUtils::FORMAT_HTML:
                ...
                $formatter->setNumCameras($numCameras) ;
                break ;
        }
