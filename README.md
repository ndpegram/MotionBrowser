  ...
  - <span><span>\[</span>disk<span>\]</span></span>  
     
      ...
      - <span>freeSpaceBuffer</span>  
        ...
        <span class="smallcaps">disk space used other than by motion (%) + free space buffer (%) + motion video recording space (%) \(\le\) 100%</span>
    
      - <span>fileAgeing</span>  
        This specifies the maximum age a video or image can be before being deleted. This is expressed as a number of days. If you wish to keep videos for a maximum of four weeks, this should be set to 28. Files younger than this will be deleted if the free space on the disk falls below the value set by <span class="smallcaps">freeSpaceBuffer</span>.
...
Added disk space reservation code. Also added code to remove files based on date.
...
# To Do/Planned features
  ...
  - Convert layout from tables to DIV and CSS format, to allow for better display on mobiles, etc., and to allow for more useful degrading. Leaving video output to table as semantically it is a table of rows and columns.
